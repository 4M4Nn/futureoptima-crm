...
function AddLeadModal({ open, onClose }) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      onClose();
      setName(""); setPhone(""); setEmail(""); setCity(""); setSource("WALK_IN"); setInterestedCourse("");
    },
    onError: (e) => {
      console.log("Error:", e);
  ...
  const handleSubmit = () => {
    ...
    mutation.mutate({ name: name.trim(), phone: phone.trim(), email: email.trim() || undefined, city: city.trim() || undefined, source: source || "OTHER", interestedCourse: interestedCourse || undefined, status: "NEW" });
  };
...
fs.writeFileSync('src/pages/leads/LeadsPage.jsx', content);
console.log('LeadsPage rewritten!');
